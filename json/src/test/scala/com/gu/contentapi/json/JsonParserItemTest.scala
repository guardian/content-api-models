...
class JsonParserItemTest extends FlatSpec with Matchers with OptionValues {
  ...
  it should "only include a content item" in {
    ...
    contentItemResponse.results should be (None)
    contentItemResponse.leadContent should be (None)
    contentItemResponse.mostViewed should be (None)
    contentItemResponse.editorsPicks should be (None)
  }
  ...
  it should "include a tag item, results, and lead content" in {
    ...
    tagItemResponse.results.map(_.length) should be (Some(10))
    tagItemResponse.leadContent.map(_.length) should be (Some(10))
    tagItemResponse.mostViewed.map(_.length) should be (None)
    tagItemResponse.editorsPicks.map(_.length) should be (None)
  }
  ...
  it should "parse tag results" in {
    tagItemResponse.results.map(_.size) should be (Some(10))
  ...
  it should "parse tag lead content" in {
    val leadContent = tagItemResponse.leadContent.get
  ...
  it should "include a section item, edition, and results" in {
    ...
    sectionItemResponse.results.map(_.length) should not be (Some(0))
    sectionItemResponse.leadContent.map(_.length) should not be (Some(0))
    sectionItemResponse.mostViewed.map(_.length) should not be (Some(0))
    sectionItemResponse.editorsPicks.map(_.length) should not be (Some(0))
  }
  ...
  it should "deserialize a package correctly" in {
    ...
    pkg.lastModified should be(capiDateTime("2014-05-21T13:40:59Z"))
